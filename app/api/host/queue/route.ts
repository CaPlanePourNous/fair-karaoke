...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ok: true,
      playing: current ? toItem(current) : null,
      waiting: waiting.map(toItem),
      done: doneRaw.map(toItem),
      // optionnel: raisons/rejets si tu veux les afficher côté Host
    ...
    // Important: on renvoie une forme valide pour ne pas faire planter l’UI
    return NextResponse.json(
