...
export async function GET(req: NextRequest) {
  try {
    ...
    const roomSlug = searchParams.get("room_slug") || "lantignie"; // défaut acceptable
    ...
    const { data: rows, error: eReq } = await db
      ...
      .eq("room_id", room_id)
      .order("created_at", { ascending: true }); // ordre brut, l’algorithme recalcule ensuite

    if (eReq) return NextResponse.json({ ok: false, error: eReq.message }, { status: 500, headers: noStore });

    const all = (rows || []) as Row[];

    // Calcul R1–R5 (+ anti-spam) — lib/ordering.ts
    // NOTE: signature: computeOrdering(requestsArray, { maxQueue?: number })
    const { orderedWaiting, rejectIds } = computeOrdering(all as any, { maxQueue: 15 });

    // Construire les listes pour l’UI
    ...
    const done    = all.filter(r => r.status === "done").sort((a, b) => {
    ...
    const playedSingerIds = new Set(all.filter(r => r.status === "done" || r.status === "playing").map(r => r.singer_id).filter(Boolean));
